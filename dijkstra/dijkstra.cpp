...
// check and read mex input
	if (nrhs != 5 || nlhs != 3)
		mexErrMsgTxt("Usage: [energy, path, correspondence] = dijkstra(vertices, triangles, contour, landmarks_2d, landmarks_3d). Transform indices in triangles to initial 0.");
    ...
    const int nLandmarks = int( mxGetN(prhs[3]) );
...
try
{
	...
	list<long> vPath;
	const double energy = dijkstra_main(vertices, nVertices, dimVertices, triangles, nTriangles, contour, nContour, dimContour, vPath, landmarks_2d, landmarks_3d, nLandmarks);
