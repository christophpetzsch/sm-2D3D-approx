...
inline double getArapCost (        
    ...
    //if(!on_optimal_path(next3dTrgt, next2dTrgt)) distDif += 200000.0;
    ...
    if (PRECOMPUTE_ROTATIONS) {
        ...
        long c2dSrc = current2dSrc == next2dSrc ? next2dSrc-1 : current2dSrc;
        if (c2dSrc < 0) c2dSrc = nVertices2d-2;
        long n2dSrc = next2dSrc == next2dTrgt ? next2dSrc-1 : next2dSrc;
        if (n2dSrc < 0) n2dSrc = nVertices2d-2;
        arapL = arapLoss(features3d,nVertices3d,dimFeatures3d,features2d,nVertices2d,dimFeatures2d,
                            c2dSrc, next2dSrc, n2dSrc, next2dTrgt, 
                            current3dSrc,current3dTrgt,next3dSrc,next3dTrgt);
    }
