...
inline void updateArrays(      const long currentIdxInLayer,
    ...
    if (result[nextIndex] < 0) {
        ...
        if(nLandmarks != 0)
        {
            if(landmarks_2d[1]==1)
            {
                if(currentLayer == 0 && (next2dSrc!=1 || current3dSrc!=landmarks_3d[0] || current3dTrgt!=landmarks_3d[1])) newValue += 1000000;
                if(next2dSrc == nContour-1 && (next3dSrc!=landmarks_3d[0] || next3dTrgt != landmarks_3d[1])) newValue += 1000000;
            }
            else
            {
                if(currentLayer == 0 && (currentIdxInLayer<4*nEdges3d || current3dTrgt != landmarks_3d[0])) newValue += 1000000;
                if(next2dSrc == nContour-2 && (next3dTrgt != landmarks_3d[0] || nextIndex < 4*nEdges3d)) newValue += 1000000;
            }
        }
        for(int i=1; i<nLandmarks; ++i)
        {
            if(landmarks_2d[i] == next2dTrgt && landmarks_3d[i] != next3dTrgt) newValue += 1000000;
        }
