...
double dijkstra_main( 	const double* 	vertices,
    ...
    const long firstIdxInLastLayer = nProductNodesPerLayer * 2;
	...
	double* const result_CG = new double[nProductNodes];
	double* const result_SG = new double[nVertices * (nContour-2)];
	long* const predecessors_CG = new long[nProductNodes];
    long* const entering_edge_SG = new long[nVertices * (nContour-2)];
    //t00 = high_resolution_clock::now();

    
    if(PRECOMPUTE_ROTATIONS && nLandmarks>0){

        int shift = landmarks_2d[0];
        int start_idx = nProductNodesPerLayer * shift;


        std::rotate(precRotations.begin(), precRotations.begin() + start_idx, precRotations.end());
        std::copy(precRotations.begin(), precRotations.begin() + nProductNodesPerLayer, precRotations.end() - nProductNodesPerLayer);

        landmarks_2d[0] = 0;
    } else if(PRECOMPUTE_ROTATIONS){
		...
		for (int j = 0; j < nVertices; ++j) {
			if (i == 0) {
                ...
                upperBoundEnergyArray[j] += getArapCost(vertices, nVertices, dimVertices, contour, nContour, dimContour, 
                            idxInLayer, idxInLayer, i-1, i, i+1, idx3dSrc, j, idx3dSrc, j, nProductNodesPerLayer,precRotations); 
    ...
    if (nLandmarks != 0) upperbound = upperBoundEnergyArray[landmarks_3d[0]];
	...
	while(!upperBounds.isEmpty() && !optimum) {
		...
		manifoldDijkstra(all_energies, upperbound, nodesPerLayer, mesh, mesh2, edges3d, vertices, nVertices, 
                         dimVertices, triangles, nTriangles, contour, nContour, dimContour,
                         result_CG, result_SG, predecessors_CG, entering_edge_SG, cuts2, cutID, precRotations, landmarks_2d, landmarks_3d, nLandmarks);
        //std::vector<int> throwVec;
        //throwVec.at(-1);
		// find minimum
		...
		for (int i = firstIdxInLastLayer; i < nProductNodes; i++) {
			...
			if (cuts2[idxInLayer] == cutID && result_CG[i] < min && result_CG[i] != -1) {
				min = result_CG[i];
				minIndex = i;
                minIndices.push_back(minIndex - firstIdxInLastLayer);
			}
            if (cuts2[idxInLayer] == cutID && result_CG[i] == -1) {
                // rule out vertices which definitely cannot be on an optimal path anymore
                cuts2[idxInLayer] = -1;
            }
            /*if (landmark_3d != -1 && getLGSrc3dVertex(i, edges3d, nVertices)!=landmark_3d){
                cuts2[idxInLayer] = -1;
            }*/
		}
        ...
        if (minIndex == -1) {
            ...
            noPathFoundCounter++;
            if (noPathFoundCounter > 20){ // make sure we do not spin forever
                break;
            }
            continue;
        }
		...
		// create path
		vPath.clear();
        calcPath(minIndex, predecessors_CG, entering_edge_SG, result_CG, result_SG, vPath, nContour, nVertices, nEdges3d, edges3d, nodesPerLayer);

        
        
        pathFront = vPath.front();
        pathBack = vPath.back()  - nContour*6*nEdges3d;//firstIdxInLastLayer;
        
        if(nLandmarks != 0) break;

        if (DEBUG_DIJKSTRA_MAIN) std::cout << "pathFront:" << pathFront  << " pathBack:" << pathBack << " Frontsourcevertex: " << getLGSrc3dVertex(pathFront, edges3d, nVertices) << " Fronttargetvertex: " << getLGTrgt3dVertex(pathFront, edges3d, nVertices) << std::endl;
		if(!vPath.empty() && pathFront == pathBack) {
            ...
            for (auto it: minIndices) {
                ...
                vPath.clear();
                calcPath(minIdxOnLastLayer, predecessors_CG, entering_edge_SG, result_CG, result_SG, vPath, nContour, nVertices, nEdges3d, edges3d, nodesPerLayer);

                const long currentPathFront = vPath.front();
                
                if (it == currentPathFront) {
                    if (result_CG[minIdxOnLastLayer] <= upperbound) {
                        upperbound = result_CG[it + firstIdxInLastLayer];
                        upperboundPath.assign(vPath.begin(), vPath.end());
                        iterOptFound = loopCounter;
                        gapOptFound = upperbound - upperBounds.peak();
    ...
    const double energy = minIndex == -1 ? upperbound : result_CG[minIndex];
	...
	delete[] result_CG;
    delete[] result_SG;
	delete[] predecessors_CG;
    delete[] entering_edge_SG;
	delete[] cuts;
    delete[] cuts2;
	delete[] all_energies;
